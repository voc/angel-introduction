...
3. Compile the slides: `./build_slides.sh` → creates two pdfs named `introduction.pdf` and `introduction_presentation.pdf` in the same folder. If you use the executable file `typst-x86_64-unknown-linux-musl`, make sure to include the full path, e.g. `~/code/tools/typst-x86_64-unknown-linux-musl compile introduction.typ`.
